...
### 9.1 The convergence rates $\alpha$, $\beta$, and $\gamma$

Theorem 2.1 in the paper summarizes MLMC efficiency using three rates. In the
paper's resolution notation, the assumptions are approximately

$$
|\mathbb{E}[Q_\ell-Q]|
\lesssim M_\ell^{-\alpha},
$$
...
$$
C_\ell\lesssim M_\ell^\gamma.
$$

Each rate answers a different question:

- $\alpha$, the **weak convergence rate**, describes how quickly the bias
  decreases. It determines how fine the final spatial level must be.
- $\beta$, the **variance or strong convergence rate**, describes how quickly
  consecutive-level differences become less random. A large $\beta$ means
  the fine corrections need relatively few samples.
- $\gamma$, the **cost growth rate**, describes how quickly one correction
  sample becomes more expensive as the spatial resolution increases.

The competition between $\beta$ and $\gamma$ determines MLMC's asymptotic
sampling cost. Under the assumptions of Theorem 2.1, the cost required to
achieve RMSE $\varepsilon$ behaves as

$$
C_\varepsilon^{\mathrm{ML}}
\lesssim
\begin{cases}
\varepsilon^{-2},
& \beta>\gamma, \\
...
The cases have a useful interpretation:

- If $\beta>\gamma$, correction variance decays faster than cost grows. The
  many samples are concentrated on cheap levels, and MLMC achieves the ideal
  Monte Carlo rate $O(\varepsilon^{-2})$, apart from constants.
- If $\beta=\gamma$, every scale contributes comparably and there is a
  logarithmic penalty.
- If $\beta<\gamma$, the fine levels remain influential because their cost
  grows faster than their correction variance decays. MLMC still improves on
  standard fine-grid Monte Carlo under the theorem's assumptions, but the
  finest levels consume more of the work.

For comparison, standard Monte Carlo on a fine discretization has cost roughly

$$
C_\varepsilon^{\mathrm{MC}}
\lesssim
\varepsilon^{-2-\gamma/\alpha}.
$$

These rate formulas are not themselves the finite-sample stopping tests. The
implementation still checks estimated sampling variance and bias. The rates
explain why the allocation works and can help diagnose an implementation by
plotting $|\overline{Y}_\ell|$, $\widehat{V}_\ell$, and
$\widehat{C}_\ell$ against resolution or level.
